function [users,items1,items2]=Sampler_LS(numSamples,nitems,nusers,row,col,xadj,adj,deg,...
    byuser,uniformsampling, MR, SIM)

% algorithm based on similarity (sample the least similar items)
...
if (uniformsampling)
    ...
    [~, indx] = sort(Select_row,2);              
    
    % choose the negative items from the negative item pools;
    % basicly, generate the negative item pool with 800 least similar items;
